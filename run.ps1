...
# ВНИМАНИЕ: scope.c разбит на два файла - scope_logic.c и scope_gui.c.
# Общий публичный заголовок остался один (scope.h), плюс появился
# приватный scope_internal.h, который подключают только эти два .c
$SRC = @(
    "src/main.c",
    "src/app.c",
    "lib/scope/scope_logic.c",
    "lib/scope/scope_gui.c",
    "lib/my_generator/my_generator.c",
...
# -------------------------
# 5.1 ПРЕДУПРЕЖДЕНИЯ КОМПИЛЯТОРА
# -------------------------
# Половина найденных багов (неинициализированные переменные, сравнение
# знакового с беззнаковым, недостижимый код после return) видна компилятору
# сразу же - надо только попросить его сказать.
#
# -Wall -Wextra почти ничего не стоят по времени сборки и ловят то,
# на поиск чего иначе уходит вечер с отладчиком.
$ARGS += "-Wall"
$ARGS += "-Wextra"

# Эти три предупреждения в текущем коде дают много шума и мало пользы:
# в GUI-коде полно заготовленных под будущее переменных и параметров
$ARGS += "-Wno-unused-parameter"
$ARGS += "-Wno-unused-variable"
$ARGS += "-Wno-unused-but-set-variable"
